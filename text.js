    ...
    if ($request.url.match(/\/kd.youth.cn\/h5\/20200612makeMoney/)) {
        ...
        if (jshb_cookie) {
            if (jshb_cookie.indexOf(bodyVal5) > -1) {
                $.log("此cookie已存在，本次跳过")
            } else if (jshb_cookie.indexOf(bodyVal5) === -1) {
                jshb_cookies = jshb_cookie + "@" + bodyVal;
                $.setdata(jshb_cookies, 'jshb_cookie');
                $.log(`${$.name}获取cookie: 成功, jshb_cookies: ${bodyVal}`);
                bodys = jshb_cookies.split("@")
                // $.msg($.name, "获取第" + bodys.length + "个阅读请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'jshb_cookie');
            $.log(`${$.name}获取cookie: 成功, jshb_cookies: ${bodyVal}`);
